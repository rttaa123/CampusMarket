...
class UsersController extends Controller
{
  ...
  public function register(Request $request)  // 注册信息验证
  {
    ...
    $this->validate(
      ...
      [
        ...
        'email' => ['required', 'regex:/^[a-zA-Z\d._%+-]+@lzu\.edu\.cn$/', 'unique:users', 'max:64'],
      ...
      [
        ...
        'email.regex' => '请填写正确的校园邮箱（@lzu.edu.cn）！',
    ...
    try {
      ...
      session()->flash('success', '注册成功~ 激活链接已发送到邮箱，请注意查收。');
      return redirect()->route('show_verify', ['user' => $user->id]);
    } catch (\Swift_TransportException $e) {
      $user->delete();
      session()->flash('error', '校园邮箱无法接收验证邮件，请确认邮箱是否正确。');
      return redirect()->back()->withInput();
    } catch (\Exception $e) {
      $user->delete();
      Log::error('Send signup email failed: '.$e->getMessage());
      session()->flash('error', '发送激活邮件失败，请稍后再试。');
      return redirect()->back()->withInput();
    }
  ...
  protected function sendSignupEmail(User $user)
  {
    $view = 'users.email.confirm';
    $subject = "感谢注册 ShopCampus 平台！请确认你的邮箱。";

    Mail::send($view, compact('user'), function ($message) use ($user, $subject) {
  ...
  public function buy_goods(User $user, Request $request)   // 订购商品-预定|已购
  {
    ...
    // 获取已购买的订单（订单对应的商品状态为已出售）
    $buy_goods = $user->buyerOrders()->whereHas('goods', function($query) {
      $query->where('state', Good::goods_state_in_sold);
    })->orderBy('created_at', 'desc');
    
    $buy_goods_count = $buy_goods->count();
    $buy_goods = $buy_goods->with('user', 'buyer', 'goods')->paginate(5);
  ...
  public function sale_goods(User $user, $state = Good::goods_state_in_release)    // 默认商品状态-预发布
  {
    ...
    $isWishMode = request()->query('mode') === 'wish';

    $goods = Good::where('user_id', $user->id)->where('state', $state);
    $goods = $this->applyGoodsTypeScope($goods, $isWishMode)->orderBy('created_at', 'desc');
    $count = $goods->count();

    //$count=sizeof($goods->get());
    $goods = $goods->paginate(5);

    $totalGoodsCount = $this->applyGoodsTypeScope(Good::where('user_id', $user->id), $isWishMode)->count();
    ...
    // 取可见数据
    $user_visible = $user->userVisibles;
    return view('users.detail._sale_goods', compact('user', 'goods', 'image', 'count', 'user_visible', 'state', 'isWishMode', 'totalGoodsCount'));
  }
  // 搜索用户出售商品，默认状态为“预发布”，请求必须在可选参数之前
  public function search_sale_goods(User $user, Request $request, $state = Good::goods_state_in_release)
  {
  ...
  public function edit_check(User $user, UserInfoRequest $request)
  {
    ...
    if (json_decode($request->email_cg) == 'true') {   // 用户修改了邮箱
      // 如果新邮箱是 @lzu.edu.cn，立即激活；否则需要验证
      if (preg_match('/@lzu\.edu\.cn$/', $request->email)) {
        // 校园邮箱立即激活
        $user->activated = true;
  ...
  public function booking_notice(User $user, Request $request)      // 预订通知
  {
    ...
    $bookings = Booking::where('user_id', $user->id)
      ->whereHas('goods', function ($goodsQuery) use ($isWishMode) {
        $this->applyGoodsTypeScope($goodsQuery, $isWishMode);
      });

    $bookingTotalCount = (clone $bookings)->count();
    ...
    if ($request->reply == 'no') {
      $no_reply_booking = (clone $bookings)->where('user_state', Booking::seller_processing_booking)->orderBy('created_at', 'desc');     // 卖家状态-待回复
      $no_reply_count = $no_reply_booking->count();    // 总数

      $no_reply_booking = $no_reply_booking->with('user', 'goods')->paginate(5);
    } elseif ($request->reply == 'yes') {
      $yes_reply_booking = (clone $bookings)->where('user_state', '!=', Booking::seller_processing_booking)->orderBy('updated_at', 'desc');    // 已回复
      $yes_reply_count = $yes_reply_booking->count();    // 总数

      $yes_reply_booking = $yes_reply_booking->with('buyer', 'goods')->paginate(5);
    }

    return view('users.detail._booking_notice', compact('user', 'no_reply_booking', 'yes_reply_booking', 'no_reply_count', 'yes_reply_count', 'isWishMode', 'bookingTotalCount'));
  }
  ...
  public function seller_order(User $user, Request $request)   // 出售订单
  {
    ...
    $type = $request->type;
    $ordersTotalCount = (clone $orders)->count();

    if ($type == 'pending') {
      $pending_orders = (clone $orders)->where('seller_state', Order::seller_pending_order)->orderBy('updated_at', 'desc');   // 卖家状态-待处理(2)，则订单都是未生效
      $pending_orders_count = $pending_orders->count();
      $pending_orders = $pending_orders->with('user', 'buyer', 'goods')->paginate(5);
    } elseif ($type == 'processed') {
      $processed_orders = (clone $orders)->where('seller_state', '!=', Order::seller_pending_order)->orderBy('updated_at', 'desc');  // 卖家状态-已处理(0-1),则考虑未生效和已生效订单
      $processed_orders_count = $processed_orders->count();
      $processed_orders = $processed_orders->with('user', 'buyer', 'goods')->paginate(5);
    }


    return view('users.detail._seller_order', compact('user', 'pending_orders_count', 'pending_orders', 'processed_orders', 'processed_orders_count', 'type', 'isWishMode', 'ordersTotalCount'));
  }
  ...
  public function search_seller_order(User $user, Request $request)
  {
    ...
    if ($type == 'pending') {
      $pending_orders = (clone $orders)->where('seller_state', Order::seller_pending_order)->where('no', 'like', $search)->orderBy('updated_at', 'desc');   // 卖家状态-待处理(2)，则订单都是未生效
      $pending_orders_count = $pending_orders->count();
      $pending_orders = $pending_orders->with('user', 'buyer', 'goods')->paginate(5);
    } elseif ($type == 'processed') {
      $processed_orders = (clone $orders)->where('seller_state', '!=', Order::seller_pending_order)->where('no', 'like', $search)->orderBy('updated_at', 'desc');  // 卖家状态-已处理(0-1),则考虑未生效和已生效订单
      $processed_orders_count = $processed_orders->count();
      $processed_orders = $processed_orders->with('user', 'buyer', 'goods')->paginate(5);
    }
  ...
  private function applyGoodsTypeScope($query, bool $isWishMode)
  {
    return $query->where(function ($typeQuery) use ($isWishMode) {
      if ($isWishMode) {
        $typeQuery->where('type', Good::TYPE_BUY);
      } else {
        $typeQuery->whereNull('type')->orWhere('type', Good::TYPE_GOOD);
      }
    });
  }
}
