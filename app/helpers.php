...
function user_center_active($type){
  ...
  if ($type === 'arrow') {
    ...
    if (((if_route('sale_goods')) || (if_route('search_sale_goods'))) && $type == 8) {
      if($isWishMode){
        return 'active';
      }
    }
    if ((if_route('booking_notice')) && $type == 9) {
      if($isWishMode){
        return 'active';
      }
    }
    if (((if_route('seller_order')) || (if_route('search_seller_order'))) && $type == 10) {
      if($isWishMode){
        return 'active';
      }
    }
