...
<div class="col-lg-6 col-md-7 col-sm-12 col-xs-12 " style="margin-left: 60px;margin-bottom:75px">
  ...
  <div class="card ">
    <div class="card-body">
      @php
        $modeLabel = $isWishMode ? '心愿' : '商品';
        $searchPlaceholder = $isWishMode ? '输入心愿信息...' : '输入商品信息...';
        $searchAction = route('search_sale_goods',[$user->id,$state]);
        if($isWishMode){
          $searchAction .= '?mode=wish';
        }
        $stateLabels = [
          0 => '预发布',
          1 => '审核中',
          2 => $isWishMode ? '展示中' : '出售中',
          3 => '预定中',
          4 => $isWishMode ? '已交易' : '已出售',
        ];
      @endphp
      <div class="row mt-2 ">
        <i class="{{ $isWishMode ? 'fas fa-heart' : 'fas fa-store' }} mr-2 ml-3 mt-2" style="font-size: 26px;color:#636b6f"></i>
        <input id="user_id" type="text"  value="{{$user->id}}" hidden data-mode="{{ $isWishMode ? 'wish' : 'shop' }}">
        <h1 class="ml-2 mt-2" style="line-height: 24px;color:#636b6f; font-size:20px;font-weight:bold; ">
          {{$user->name}}
          <span style="letter-spacing:2px">发布的{{ $modeLabel }}</span>
      ...
      <form class="form-inline mt-2 ml-2" method="get" action="{{ $searchAction }}">
        <input class="form-control mr-sm-2" name="content" type="search" placeholder="{{ $searchPlaceholder }}" aria-label="Search" style="width: 300px;">
            ...
            <li class="list-group-item">
              <div class="row no-gutters">
                ...
                <div class="card-body" style="padding: 0.8rem;">
                  ...
                  <div class="mt-3">
                    <span class="" style="color: #E23729;hegiht:25px;margin-bottom:0" title="售价">
                      ￥{{ $value->price }}元
                    </span>
                    <span class="ml-4">
                      <small title="{{ $value->created_at }}" class="text-muted">发布于
...
<script>
  $(document).ready(function() {
    user_id=$('#user_id').val()
    var wishlistMode = $('#user_id').data('mode');
    var modeQuery = wishlistMode === 'wish' ? '?mode=wish' : '';

    // 商品状态-点击
    $('#unput-tab').click(function() { // 未发布
      window.location.href = "/users/"+user_id+"/sale_goods/0"+modeQuery;
    })
    $('#check-tab').click(function() { // 审核ing
      window.location.href = "/users/"+user_id+"/sale_goods/1"+modeQuery;
    })
    $('#onsale-tab').click(function() { //出售ing
      window.location.href = "/users/"+user_id+"/sale_goods/2"+modeQuery;
    })
    $('#booking-tab').click(function() { // 预定
      window.location.href = "/users/"+user_id+"/sale_goods/3"+modeQuery;
    })
    $('#alput-tab').click(function() { // 已出售
      window.location.href = "/users/"+user_id+"/sale_goods/4"+modeQuery;
    })
