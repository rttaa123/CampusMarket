...
class GoodsController extends AdminController
{
  ...
  protected function grid()
  {
    ...
    $grid->model()->with(['category', 'categories']);
    ...
    $grid->column('categories', '分区')->display(function () {
  ...
  protected function detail($id)
  {
    ...
    $show->field('categories', '分区')->as(function () {
      $names = $this->category_names ?? [];
      if (empty($names)) {
        return '-';
      }
  ...
  protected function form()
  {
    ...
    $form->saving(function (Form $form) {
      ...
      // 从请求中获取所有数据，包括 check 字段
      $allData = request()->all();
      
      // 调试：记录提交的数据（生产环境可删除）
      Log::info('Goods check form data:', $allData);
      
      // 移除虚拟字段，避免尝试保存到数据库
      $form->ignore(['check', 'reason', 'other_reason']);
      
      // 从多个可能的位置获取 check 值
      $checkValue = $allData['check'] ?? $allData['_check'] ?? request()->input('check') ?? null;
      
      // 如果 check 字段存在，根据其值设置 state
      if ($checkValue !== null && $checkValue !== '') {
        // switch 的 states 配置：'on' => value=2（出售中），'off' => value=0（预发布）
        // 但实际提交的值可能是数字 2/0，也可能是字符串 'on'/'off'，或者是 '1'/'0'
        $checkValue = is_string($checkValue) ? trim($checkValue) : $checkValue;
        
        if ($checkValue == Good::goods_state_in_selling || $checkValue === '2' || $checkValue === 2 || $checkValue === 'on' || $checkValue === '1') {
          // 审核通过
          $good->state = Good::goods_state_in_selling;
          Log::info('Setting goods state to selling (2) for goods ID: ' . $good->id);
        } elseif ($checkValue == Good::goods_state_in_release || $checkValue === '0' || $checkValue === 0 || $checkValue === 'off') {
          // 审核不通过
          $good->state = Good::goods_state_in_release;
          Log::info('Setting goods state to release (0) for goods ID: ' . $good->id);
        }
        
        // 确保 state 被设置到模型
        $form->model()->state = $good->state;
      } else {
        Log::warning('Check value is null or empty for goods ID: ' . $good->id);
      }
    });
    ...
    $form->saved(function (Form $form) {
      $good = $form->model();
      $checkValue = request()->input('check');
      ...
      if ($checkValue !== null) {
        ...
        if ($checkValue == Good::goods_state_in_selling || $checkValue === '2' || $checkValue === 2 || $checkValue === 'on') {
          // 审核通过
          $notifyData['is_check'] = 'true';
        } else {
          // 审核不通过
          $notifyData['is_check'] = 'false';
          
          // 处理拒绝原因
          $reason = '';
          if (isset($data['reason']) && is_array($data['reason'])) {
            foreach ($data['reason'] as $key => $value) {
              if ($value && isset(Good::$checkReason[$key])) {
                if ($reason) {
                  $reason .= '，';
                }
                $reason .= Good::$checkReason[$key];
              }
            }
          }
          if (isset($data['other_reason']) && !empty($data['other_reason'])) {
            if ($reason) {
              $reason .= '，';
            }
        ...
        // 发送通知
        try {
          $good->user->notify(new CheckGoods($good, $notifyData));
        } catch (\Exception $e) {
          // 通知发送失败不影响审核
        }
      }
  ...
  public function update($id)
  {
    ...
    // 检查商品是否处于待审核状态
    if ($good->state != Good::goods_state_in_check) {
      admin_toastr('该商品不是待审核状态', 'error');
      return redirect()->back();
    }
    ...
    // 记录日志
    Log::info('Goods check update data:', $data);
    
    // 获取 check 字段的值
    $checkValue = $data['check'] ?? null;
    ...
    if ($checkValue !== null && $checkValue !== '') {
      ...
      if ($checkValue == Good::goods_state_in_selling || $checkValue === '2' || $checkValue === 2 || $checkValue === 'on' || $checkValue === '1') {
        ...
        admin_toastr('审核不通过', 'warning');
      } else {
        admin_toastr('审核状态值无效', 'error');
        return redirect()->back();
      }
