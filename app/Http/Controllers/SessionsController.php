...
class SessionsController extends Controller
{
  ...
  {  
    // 允许 @qq.com 和 @lzu.edu.cn 邮箱登录（兼容已注册用户）
    $credentials = $this->validate($request, [
      'email' => ['required', 'regex:/^[a-zA-Z\d]+(@qq\.com|@lzu\.edu\.cn)$/', 'max:64','exists:users,email'],
      'password' => 'required',
    ], [
      'email.regex' => '请填写正确的邮箱格式！',
    ...
    if ($user->activated) {
      ...
      $subject = "感谢注册 Shop-Campus 平台！请确认你的邮箱。";
  ...
  {
    $user = User::where('activation_token', $token)->where('email',$request->email)->first();

    if(!$user){
      session()->flash('error', '激活链接无效或邮箱不匹配，请重新注册或重新发送验证邮件。');
      return redirect()->route('login');
    }
  ...
  {     
    // 允许 @qq.com 和 @lzu.edu.cn 邮箱（兼容已注册用户）
    $credentials = $this->validate($request, [
      'email' => ['required', 'regex:/^[a-zA-Z\d]+(@qq\.com|@lzu\.edu\.cn)$/', 'max:64', 'exists:users,email'],
